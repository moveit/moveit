...
PLUGINLIB_EXPORT_CLASS(stomp_moveit::cost_functions::ObstacleDistanceGradient,
...
{
ObstacleDistanceGradient::ObstacleDistanceGradient() : name_("ObstacleDistanceGradient"), robot_state_()
{
}
...
bool ObstacleDistanceGradient::configure(const XmlRpc::XmlRpcValue& config)
{
  try
  {
    ...
    longest_valid_joint_move_ = c.hasMember("longest_valid_joint_move") ?
                                    static_cast<double>(c["longest_valid_joint_move"]) :
                                    LONGEST_VALID_JOINT_MOVE;

    if (!c.hasMember("longest_valid_joint_move"))
    {
      ROS_WARN("%s using default value for 'longest_valid_joint_move' of %f", getName().c_str(),
               longest_valid_joint_move_);
    }
  }
                                            ...
                                            Eigen::VectorXd& costs, bool& validity)
{
  ...
  for (auto t = start_timestep; t < start_timestep + num_timesteps; ++t)
  {
    if (!skip_next_check)
    {
      ...
      planning_scene_->checkSelfCollision(collision_request_, collision_result_, *robot_state_,
...
bool ObstacleDistanceGradient::checkIntermediateCollisions(const Eigen::VectorXd& start, const Eigen::VectorXd& end,
