 ...
 */
static Eigen::MatrixXd jointsToToolPath(RobotState& state, const std::string& group_name,
...
TrajectoryVisualization::TrajectoryVisualization()
  : name_("TrajectoryVisualization"), nh_("~"), line_width_(0.0), publish_intermediate_(false)
{
                                                   ...
                                                   moveit_msgs::MoveItErrorCodes& error_code)
{
  ...
  // initializing marker
  createToolPathMarker(tool_traj_line_, MARKER_ID, robot_model_->getRootLinkName(), rgb_, line_width_,
  ...
  visualization_msgs::Marker m;
  createToolPathMarker(Eigen::MatrixXd(), MARKER_ID, robot_model_->getRootLinkName(), rgb_, line_width_,
                       marker_namespace_, m);
  m.action = m.DELETE;
  viz_pub_.publish(m);
...
bool TrajectoryVisualization::filter(std::size_t start_timestep, std::size_t num_timesteps, int iteration_number,
