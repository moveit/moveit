...
{
ConstrainedCartesianGoal::ConstrainedCartesianGoal() : name_("ConstrainedCartesianGoal")
{
...
bool ConstrainedCartesianGoal::configure(const XmlRpc::XmlRpcValue& config)
{
  ...
  try
  {
    ...
    if ((dof_nullity_param.getType() != XmlRpcValue::TypeArray) || dof_nullity_param.size() < CARTESIAN_DOF_SIZE ||
        dof_thresholds_param.getType() != XmlRpcValue::TypeArray || dof_thresholds_param.size() < CARTESIAN_DOF_SIZE ||
        joint_updates_param.getType() != XmlRpcValue::TypeArray || joint_updates_param.size() == 0)
    {
...
bool ConstrainedCartesianGoal::filter(std::size_t start_timestep, std::size_t num_timesteps, int iteration_number,
                                      const Eigen::MatrixXd& parameters, Eigen::MatrixXd& updates, bool& filtered)
{
  ...
  if (kinematics::computeJacobianNullSpace(state_, group_name_, tool_link_, kc_.constrained_dofs, kc_.init_joint_pose,
                                           jacb_nullspace))
  {
