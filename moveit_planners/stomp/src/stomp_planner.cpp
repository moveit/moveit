 ...
 */
bool parseConfig(XmlRpc::XmlRpcValue config, const moveit::core::JointModelGroup* group,
...
StompPlanner::StompPlanner(const std::string& group, const XmlRpc::XmlRpcValue& config,
  ...
  : PlanningContext(DESCRIPTION, group), config_(config), robot_model_(model), ph_(new ros::NodeHandle("~"))
{
  setup();
}
...
void StompPlanner::setup()
{
  ...
  try
  {
    ...
    if (!config_.hasMember("optimization") ||
        !parseConfig(config_["optimization"], robot_model_->getJointModelGroup(group_), stomp_config_))
    {
...
bool StompPlanner::solve(planning_interface::MotionPlanDetailedResponse& res)
{
  ...
  ROS_WARN_COND(TIMEOUT_INTERVAL > request_.allowed_planning_time,
                "%s allowed planning time %f is less than the minimum planning time value of %f", getName().c_str(),
                request_.allowed_planning_time, TIMEOUT_INTERVAL);
  ros::Timer timeout_timer = ph_->createTimer(ros::Duration(TIMEOUT_INTERVAL),
                                              [&](const ros::TimerEvent& evnt) {
                                                if ((ros::WallTime::now() - start_time) > allowed_time)
                                                {
                                                  ROS_ERROR("%s exceeded allowed time of %f , terminating",
                                                            getName().c_str(), allowed_time.toSec());
                                                  this->terminate();
                                                }
...
bool StompPlanner::jointTrajectorytoParameters(const trajectory_msgs::JointTrajectory& traj,
...
bool StompPlanner::extractSeedTrajectory(const moveit_msgs::MotionPlanRequest& req,
                                         trajectory_msgs::JointTrajectory& seed) const
{
  ...
  for (size_t i = 0; i < constraints.size(); ++i)
  {
    ...
    for (size_t j = 0; j < constraints[i].joint_constraints.size(); ++j)
    {
      ...
      {
        ROS_WARN("Seed trajectory (index %lu, joint %lu) joint name '%s' does not match expected name '%s'", i, j,
...
bool StompPlanner::getConfigData(ros::NodeHandle& nh, std::map<std::string, XmlRpc::XmlRpcValue>& config,
                                 std::string param)
{
