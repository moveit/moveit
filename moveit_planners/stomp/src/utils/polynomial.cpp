...
bool applyPolynomialSmoothing(moveit::core::RobotModelConstPtr robot_model, const std::string& group_name,
                              Eigen::MatrixXd& parameters, int poly_order, double joint_limit_margin)
{
  using namespace Eigen;
  using namespace moveit::core;

  const std::vector<const JointModel*>& joint_models =
      robot_model->getJointModelGroup(group_name)->getActiveJointModels();
