 ...
 */
#ifndef MOVEIT_STOMP_MOVEIT_INCLUDE_STOMP_MOVEIT_UTILS_KINEMATICS_H_
#define MOVEIT_STOMP_MOVEIT_INCLUDE_STOMP_MOVEIT_UTILS_KINEMATICS_H_
...
struct KinematicConfig
{
   ...
   */
  Eigen::ArrayXd joint_update_rates = Eigen::ArrayXd::Zero(
      1, 1); /**< @brief The weights to be applied to each update during every iteration [num_dimensions x 1]. **/
  Eigen::Array<double, 6, 1> cartesian_convergence_thresholds =
      Eigen::Array<double, 6,
                   1>::Zero(); /**< @brief  The error margin for each dimension of the twist vector [6 x 1]. **/
  Eigen::VectorXd init_joint_pose = Eigen::ArrayXd::Zero(1, 1); /**< @brief  Seed joint pose [num_dimension x 1]. **/
  int max_iterations =
      100; /**< @brief  The maximum number of iterations that the algorithm will run until convergence is reached. **/

  /** @name Null Space Parameters
   * Used in exploding the task manifold null space
   */
  Eigen::ArrayXd null_proj_weights =
      Eigen::ArrayXd::Zero(1, 1); /**< @brief Weights to be applied to the null space vector */
  Eigen::VectorXd null_space_vector =
      Eigen::VectorXd::Zero(1); /**< @brief Null space vector that is used to exploit the Jacobian's null space */
};
 ...
 * @param oc                The orientation constraint message.  Absolute tolerances greater than 2PI will be considered
 * unconstrained.
 * @param init_joint_pose   The initial joint values
 * @param kc                The output KinematicConfig object
 * @return  True if succeeded, false otherwise
 */
static bool createKinematicConfig(const moveit::core::JointModelGroup* group, const moveit_msgs::PositionConstraint& pc,
                                  const moveit_msgs::OrientationConstraint& oc, const Eigen::VectorXd& init_joint_pose,
                                  KinematicConfig& kc)
{
...
/**
 * @brief Computes the twist vector [vx vy vz wx wy wz]'  relative to the current tool coordinate system.  The
 * rotational part is
...
/**
 * @brief Solves the inverse kinematics for a given tool pose using a gradient descent method.  It can handle under
 * constrained DOFs for
 *  the cartesian tool pose and can also apply a vector onto the null space of the jacobian in order to meet a secondary
 * objective.  It
 *  also checks for joint limits.
 * @param robot_state                       A pointer to the robot state.
 * @param group_name                        The name of the kinematic group. The tool link name is assumed to be the
 * last link in this group.
 * @param constrained_dofs                  A vector of the form [x y z rx ry rz] filled with 0's and 1's to indicate an
 * unconstrained or fully constrained DOF.
 * @param joint_update_rates                The weights to be applied to each update during every iteration
 * [num_dimensions x 1].
 ...
 * @param null_space_vector                 The null space vector which is applied into the jacobian's null space
 * [num_dimension x 1].
 * @param max_iterations                    The maximum number of iterations that the algorithm will run until
 ...
 */
static bool solveIK(moveit::core::RobotStatePtr robot_state, const std::string& group_name,
                    const KinematicConfig& config, Eigen::VectorXd& joint_pose)
{
  return solveIK(robot_state, group_name, config.constrained_dofs, config.joint_update_rates,
                 config.cartesian_convergence_thresholds, config.null_proj_weights, config.null_space_vector,
                 config.max_iterations, config.tool_goal_pose, config.init_joint_pose, joint_pose);
}

/**
 * @brief Convenience function to calculate the Jacobian's null space matrix for an under constrained tool cartesian
 * pose.
 * @param state             A pointer to the robot state.
 * @param group             The name of the kinematic group.
 * @param tool_link         The tool link name
 * @param constrained_dofs  A vector of the form [x y z rx ry rz] filled with 0's and 1's to indicate an unconstrained
 * or fully constrained DOF.
 ...
 * [num_dimensions x 1].
 * @param cartesian_convergence_thresholds  The error margin for each dimension of the twist vector [6 x 1].
 * @param max_iterations                    The maximum number of iterations that the algorithm will run to reach
 * convergence.
...
#endif /* MOVEIT_STOMP_MOVEIT_INCLUDE_STOMP_MOVEIT_UTILS_KINEMATICS_H_ */
